...
class SecureLoopController {
    ...
    public function startServer(){
        if(empty($this->ioserver)){
            ...
            $secureWebSock = new \React\Socket\SecureServer($webSock, $this->loop, [
                'local_cert' => dirname(__DIR__) . '/ssl/local.pem',
                'local_pk' => dirname(__DIR__) . '/ssl/local-key.pem',
                'allow_self_signed' => true,
                'verify_peer' => false
            ]);


            $secureWebSock->listen($this->bindPort, $this->bindIp);
